...
void step(double* const f1, double* const f0, const double dt, 
	  const double dx, const double D, const int N) {

    f1[0] = D/(dx*dx) * (f0[1]-2*f0[0]+f0[N-1])*dt + f0[0];
    f1[N-1] = D/(dx*dx) * (f0[0]-2*f0[N-1]+f0[N-2])*dt + f0[N-1];
    
    for(int i=1; i<N-1; i++)
	f1[i] = D/(dx*dx) * (f0[i+1]-2*f0[i]+f0[i-1])*dt + f0[i];
}
